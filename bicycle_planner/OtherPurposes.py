...
processing.run("native:multiparttosingleparts", {'INPUT':'/Users/laurentcazor/Documents/Trivector work/Work destination choice/Test_small/small_net.shp','OUTPUT':'/Users/laurentcazor/Documents/Trivector work/Work destination choice/Test_small/small_net_s.shp'})
...
processing.run("native:centroids", {'INPUT':deso,'ALL_PARTS':False,'OUTPUT':'/Users/laurentcazor/Documents/Trivector work/Work destination choice/Test_small/origins.shp'})
...
leisure = [ 'attraction' , 'cinema' , 'community_centre' , 'dog_park', 'garden_centre' , 'golf_course' , 'museum' , 'park' , 'picnic_site' , 'pitch' , 'playground' , 'sports_centre' , 'theatre' , 'track'  ]
shopping = [ 'bicycle_shop' , 'clothes' , 'gift_shop', 'mobile_phone_shop', 'outdoor_shop' , 'supermarket' , 'toy_shop' , 'video_shop']
services = [ 'atm' , 'bank' , 'bakery' , 'cafe' , 'car_dealership' , 'car_wash' , 'car_rental' , 'dentist' , 'fast_food', 'hairdresser' , 'kindergarten' , 'kiosk' , 'laundry' , 'library' ,  'pharmacy' , 'police' , 'post_box' , 'post_office' , 'pub' , 'recycling' , 'recycling_paper' , 'restaurant' , 'school', 'toilet' , 'town_hall' , 'veterinary'  ]
touring = [ 'artwork' ,  'chalet' , 'castle' , 'camp_site' , 'fountain' , 'hostel' , 'hotel', 'ruins' , 'tourist_info' , 'tower', 'viewpoint']
...
for i in range (len(purp)):
    ...
    dest_p = '/Users/laurentcazor/Documents/Trivector work/Work destination choice/Test_small/small_poi'+name+'.shp'
    writer = QgsVectorFileWriter.writeAsVectorFormat(layer_poi,dest_p,"utf-8",layer_poi.crs(),"ESRI Shapefile",onlySelected=True)
...
# 3.3 Merge layers
    processing.run("native:union", {'INPUT':dest_p,'OVERLAY':origins,'OVERLAY_FIELDS_PREFIX':'D_','OUTPUT':'/Users/laurentcazor/Documents/Trivector work/Work destination choice/Test_small/OD_'+name+'.shp'})
    OD_p = iface.addVectorLayer('/Users/laurentcazor/Documents/Trivector work/Work destination choice/Test_small/OD_'+name+'.shp','','ogr')
    with edit(OD_p):
...
# 4. Creation of a relation layer: origins and destinations which are not too far
    ...
    processing.run("saga:pointdistances", {'POINTS':origins,'ID_POINTS':'ID','NEAR':dest_p,'ID_NEAR':'ID','FORMAT':1,'MAX_DIST':25000,'DISTANCES':'/Users/laurentcazor/Documents/Trivector work/Work destination choice/Test_small/Relations_'+name+'.dbf'})
